...
var collectTotalCount = function (
  company,
  tenant,
  businessUnit,
  window,
  ...
  countService
    .OnGetTotalCountLib(tenant, company, businessUnit, window, "*", "*")
    .then(function (result) {
      reply.DashboardData.TotalCountWindow = result.value;
      return countService.OnGetTotalCountLib(
        tenant,
        company,
        businessUnit,
        window,
...
var collectCurrentCount = function (
  company,
  tenant,
  businessUnit,
  window,
  ...
  countService
    .OnGetCurrentCountLib(tenant, company, businessUnit, window, "*", "*")
    .then(function (result) {
      reply.DashboardData.CurrentCountWindow = result.value;
      return countService.OnGetCurrentCountLib(
        tenant,
        company,
        businessUnit,
        window,
...
var collectTotalTime = function (
  ...
  countService
    .OnGetTotalTimeLib(tenant, company, businessUnit, window, "*", "*")
    .then(function (result) {
      reply.DashboardData.TotalTimeWindow = result.value;
      return countService.OnGetTotalTimeLib(
        tenant,
        company,
        businessUnit,
        window,
...
var collectMaxTime = function (
  ...
  countService
    .OnGetMaxTimeLib(tenant, company, businessUnit, window, "*", "*")
    .then(function (result) {
      reply.DashboardData.MaxTimeWindow = result.value;
      return countService.OnGetMaxTimeLib(
        tenant,
        company,
        businessUnit,
        window,
...
var collectCurrentMaxTime = function (
  company,
  tenant,
  businessUnit,
  window,
  ...
  countService
    .OnGetCurrentMaxTimeLib(tenant, company, businessUnit, window, "*", "*")
    .then(function (result) {
      reply.DashboardData.CurrentMaxTimeWindow = result.value;
      return countService.OnGetCurrentMaxTimeLib(
        tenant,
        company,
        businessUnit,
        window,
...
var collectAverageTime = function (
  company,
  tenant,
  businessUnit,
  window,
  ...
  countService
    .OnGetAverageTimeLib(tenant, company, businessUnit, window, "*", "*")
    .then(function (result) {
      reply.DashboardData.AverageTimeWindow = result.value;
      return countService.OnGetAverageTimeLib(
        tenant,
        company,
        businessUnit,
        window,
...
var collectQueueDetails = function (
  company,
  tenant,
  businessUnit,
  window,
  ...
  countService
    .OnGetQueueDetailsLib(tenant, company, businessUnit)
    .then(function (result) {
...
var collectSingleQueueDetails = function (
  company,
  tenant,
  businessUnit,
  window,
  ...
  logger.info(
    "DVP-DashboardService.collectSingleQueueDetails Internal method "
  );
  ...
  countService
    .OnGetSingleQueueDetailsLib(tenant, company, businessUnit, queueId)
    .then(function (result) {
...
var collectTotalKeyCount = function (
  company,
  tenant,
  businessUnit,
  window,
  ...
  countService
    .OnGetCountPerKeyLib(tenant, company, businessUnit, window, "*", "*")
    .then(function (result) {
      reply.DashboardData.TotalCountWindow = result.value;
      return countService.OnGetCountPerKeyLib(
        tenant,
        company,
        businessUnit,
        window,
...
var collectTotalTimeWithCurrentSession = function (
  company,
  tenant,
  businessUnit,
  window,
  ...
  logger.info(
    "DVP-DashboardService.collectTotalTimeWithCurrentSession Internal method "
  );
  ...
  countService
    .OnGetTotalTimeWithCurrentSessionsLib(
      tenant,
      company,
      businessUnit,
      window,
    ...
    .then(function (result) {
      reply.DashboardData.TotalTimeWindow = result.value;
      return countService.OnGetTotalTimeWithCurrentSessionsLib(
        tenant,
        company,
        businessUnit,
        window,
...
var publishResetAll = function (req, res) {
  ...
  var reply = {
    ...
    DashboardData: {
      businessUnit:
        req.params.businessUnit === "*" ? "all" : req.params.businessUnit,
      window: "DASHBOARD",
      param1: "RESETALL",
      param2: "RESETALL",
    },
...
var publishDashboardData = function (req, res) {
  ...
  if (
    req.params &&
    req.params.window == "DASHBOARD" &&
    req.params.param1 == "RESETALL"
  ) {
    var reply = {
      roomData: {
        roomName: req.params.window + ":" + req.params.param1,
        eventName: req.params.param1,
      },
      DashboardData: {
        businessUnit:
          req.params.businessUnit === "*" ? "all" : req.params.businessUnit,
      ...
      .then(function (dbPubMeta) {
        if (dbPubMeta) {
          ...
          dbPubMeta.forEach(function (pubMeta) {
            switch (pubMeta.EventName) {
              ...
              case "TotalTimeWithCurrentSession":
                asyncFuncArray.push(
                  collectTotalTimeWithCurrentSession(
                    company,
                    tenant,
                    req.params.businessUnit,
                    req.params.window,
          ...
          if (asyncFuncArray && asyncFuncArray.length > 0) {
            Q.all(asyncFuncArray)
              .then(function (results) {
                if (results) {
                  results.forEach(function (result) {
                    var postData = {
                      message: result.DashboardData,
                      From: "DashboardService",
                    };
                    RequestToNotify(
                      company,
                      tenant,
                      result.roomData.roomName,
                      result.roomData.eventName,
                      postData
                    );
                  });
                }
